...
function HeroSection({img, title, overview, id}: { img: string, title: string, overview: string, id: number }) {
    ...
    return (
        <NavLink
            to={`/movie/${id}`}
            className="w-full h-96 shadow-xl rounded-lg relative px-8"
            >
            <img src={img} alt="Hero Section" className="w-full h-full object-cover shadow rounded opacity-75"/>
            ...
            <div className="bg-black opacity-75 rounded-tr-md absolute bottom-0 left-8 w-1/2">
                <div className="w-full flex flex-col items-end justify-end p-6">
                    <p className="text-white text-center text-xs">{overview}</p>
                </div>
            </div>
        </NavLink>
    );
